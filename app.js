...
const App = {
    ...
    saveSettings() {
        ...
        const parseVal = (id) => {
            ...
            return (val !== '' && !isNaN(val)) ? Math.max(1, parseInt(val)) : null;
        };

        let settings = { "smokeMode": sMode, "drinkMode": dMode };
        const addIfSet = (key, id) => { let v = parseVal(id); if (v !== null) settings[key] = v; };
        addIfSet('smokePrice', 'setSmokePrice');
        addIfSet('smokePerDay', 'setSmokePerDay');
        addIfSet('smokeTarget', 'setSmokeTarget');
        addIfSet('drinkCost', 'setDrinkCost');
        addIfSet('drinkPerWeek', 'setDrinkPerWeek');
        addIfSet('drinkTarget', 'setDrinkTarget');
